import React from 'react';

const Meet = () => {
    return (
        <div className="all-center w-full z-20 md:pl-40 pl-0">
            <div className="flex-col m-auto md:max-w-full max-w-[80%]">
                <h1 className="text-title-box text-white md:text-start text-center md:text-5xl text-6xl">ABOUT $DOGECOIN</h1>
                <p className="text-content-box md:!text-start !text-justify  md:text-black text-white md:-mt-3 mt-4">
                    Originally formed as a "joke", Dogecoin was created by IBM software engineer Billy Markus and Adobe software engineer Jackson Palmer. 
                    They wanted to create a peer-to-peer digital currency that could reach a broader demographic than Bitcoin. 
                    In addition, they wanted to distance it from the controversial history of other coins. 
                    Dogecoin was officially launched on December 6, 2013, and within the first 30 days, there were over a million visitors to Dogecoin.com.
                    In addition, they wanted to distance it from the controversial history of other coins. Dogecoin was officially launched on December 6, 2013, and within the first 30 days, there were over a million visitors to Dogecoin.com.
                </p>
            </div>
        </div>
    );
};
export default Meet;
